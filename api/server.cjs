...
// delete the BELOW. keep the ABOVE.
...
// Log specifically to Vercel logs
app.use((req, res, next) => {
  console.log(`[VERCEL LOG] Incoming ${req.method} request to ${req.url}`);
  next();
});

// The Route
app.post('/api/chat', (req, res) => {
  console.log('[VERCEL LOG] Chat route hit!');
  res.json({ 
    message: "Connection successful!", 
    reply: "I am a dummy bot. If you see this, the Vercel API is working!" 
  });
});

app.get('/api/chat', (req, res) => {
    res.json({ message: "GET request received. Use POST to chat." });
});
